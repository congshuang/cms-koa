<template>
  <div class="zj">
    <div class="box-title" style="width: 100%;" v-if="isShowBtn">
      <el-button-group>
        <el-button type="primary" size="small" @click="approve" icon="icon iconfont icon-shenhe">审 核</el-button>
        <el-button type="primary" size="small" @click="toAdd" icon="icon iconfont icon-add" v-has="`prodetection:whole:save`">新增整体环境试验</el-button>
        <el-button
          size="small"
          type="primary"
          @click.prevent.stop="handlechange2()" icon="icon iconfont icon-edit" v-has="`prodetection:whole:delete`">修 改
        </el-button>
        <el-button
          size="small"
          type="primary"
          @click.prevent.stop="handleDelete()" icon="icon iconfont icon-delete" v-has="`prodetection:whole:update`">删 除
        </el-button>
      </el-button-group>
    </div>
    <el-dialog v-dialogDrag title="选择审核人" :visible.sync="componentFixVisible" width="600px">
      <component is="flow-zj-dialog" :busiDataId="busiDataId" :flowId="flowId" :formData="formList" :flagtag="flagtag"
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <span v-if="scope.row.dataStatus == '3'"
                style="color:#409EFF;">{{getstate(scope.row.dataOperType)}}  审批未通过</span>
          <span v-if="scope.row.dataStatus == '4'"
                style="color:#f56c6c;">{{getstate(scope.row.dataOperType)}}  未提交</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="secretRank"
...
<script type="text/javascript">
  ...
  import flowZjDialog from "../../components/dialog/flowZjDialog";
  ...
  export default {
    ...
    methods: {
      ...
      ...mapMutations({
        setInfoZid: types.SET_INFO_ZJID,
        setAppInfo: types.SET_APP_INFO
      }),
      handleSelectionChange(val) {
        this.multipleSelection = val;
        if(!this.isshow){
          this.$emit('childByValueZj',this.multipleSelection);
        }
      },
      //审核
      approve() {
        ...
        this.$http.post(api.SYS_applyBefore_checkNeedAudit, {
          ...
          if (flag && !istrue) {
            ...
            this.$confirm('确定是否删除?', '提示', {
              ...
              this.$http.post(`${api.pimWholeDetectionRecord_delete}/${tag}`, row).then((res) => {
                this.loadData();
                if (flag) {
      ...
      loadData() {
        ...
        let url = "";
        if(this.isshow){
          url =`${api.pimWholeDetectionRecord_list}`;
        }else{
          url =`${api.WholerecordCompared}${this.id}`;
        }
        this.$http.post(url, {
          query: {
