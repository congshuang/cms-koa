  ...
  <div class="dj">
    ...
    <div class="box-title" style="width: 100%;" v-if="isShowBtn">
      <el-button-group>
        ...
        <el-button type="primary" icon="icon iconfont icon-add" size="small" @click="toAdd" v-has="`product:regularJointTest:save`">新增联式记录</el-button>
        <el-button
          size="small"
          type="primary"
          @click.prevent.stop="handlechange2()" icon="icon iconfont icon-edit" v-has="`product:regularJointTest:update`">修 改
        </el-button>
        <el-button
          size="small"
          type="primary"
          @click.prevent.stop="handleDelete()" icon="icon iconfont icon-delete" v-has="`product:regularJointTest:delete`">删 除
        </el-button>
      </el-button-group>
    </div>
    <el-dialog v-dialogDrag title="选择审核人" :visible.sync="componentFixVisible" width="600px">
      <component is="flow-ls-dialog" :busiDataId="busiDataId" :flowId="flowId" :formData="formList" :flagtag="flagtag"
  ...
  export default {
    ...
    methods: {
        ...
        this.$http.post(api.SYS_applyBefore_checkNeedAudit,{
          ...
          if (flag&&!istrue) {
            ...
            this.$confirm('确定是否删除?', '提示', {
              ...
              this.$http.post(`${api.regularTestdelete}/${tag}`, row).then((res) => {
                this.loadData();
                if(flag){
      ...
      loadData() {
        ...
        this.$http.post(`${api.JointTestsearch}`,{
          query: {
